...
const char *ShaderManager::eglGetErrorStr()
{
    switch (eglGetError())
    {
    case EGL_SUCCESS:
...
void ShaderManager::TestProgram() {
    // run GL program?
...
void ShaderManager::InitFreetype() {
    ...
    {
        LOG_ERR << "Could not init FreeType Library" << std::endl;
        return;
    }

    FT_Face face;
    if (FT_New_Face(ft, "/home/andrew/codac/font/LibertinusSerifDisplay-Regular.ttf", 0, &face))
    {
        LOG_ERR << "Failed to load font" << std::endl;  
        return;
    }
    ...
    for (unsigned char c = 0; c < 128; c++)
    {
        ...
        {
            LOG_ERR << "Failed to load Glyph" << std::endl;
            continue;
        }
        // generate texture
    ...
    // Unload Freetype once textures have been created
    FT_Done_Face(face);
    FT_Done_FreeType(ft);
